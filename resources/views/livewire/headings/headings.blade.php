            ...
            <div class="bg-white dark:bg-gray-800 relative border rounded-lg overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tr>
                            <th scope="col" class="px-4 py-3 border border-gray-400 border-l-0">Заголовок</th>
                            <th scope="col" class="px-4 py-3 border border-gray-400 text-center">Раздел</th>
                            <th scope="col" class="px-4 py-3 border border-gray-400 text-center">Кол. рецептов</th>
                            <th scope="col" class="px-4 py-3 border border-gray-400 text-center">Тип</th>
                            <th scope="col" class="px-4 py-3 border border-gray-400 text-center">Дата</th>
                            <th scope="col" class="px-4 py-3 border border-gray-400 border-r-0 w-fit"></th>
                        </tr>
                        </thead>
                        <tbody>
                        @if(count($headings) > 0)
                            @foreach ($headings as $k => $heading)
                                <tr wire:key="{{ $heading->id }}"
                                    ...
                                    <td class="px-4 py-3 w-1/2">{{ $heading->name }}</td>
                                    <td class="px-4 py-3 border text-center w-fit">{{ $heading->parent_sect }}</td>
                                    <td class="px-4 py-3 border text-center">{{ $heading->col_recipe }}</td>
                                    <td class="px-4 py-3 border text-center">{{ $heading->type == 1 ? 'по параметрам' : 'ручная' }}</td>
                                    <td class="px-4 py-3 border text-center">{{ $heading->updated_at->format("d.m.Y") }}</td>
                                    <td class="px-4 py-3 flex m-w-[80px]">
