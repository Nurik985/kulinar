    ...
    <form wire:submit="updateRecipe" class="space-y-6" onkeydown="enterRemove(event, event.key)">
        <div class="w-full rounded-[4px] mr-3 mb-10 border bg-white dark:bg-gray-700">
            <div class="px-6 py-6 lg:px-8">
                <div class="grid gap-2 grid-cols-custom2 sm:gap-4 relative">
                    <div class="col-span-1">
                        ...
                        <div wire:ignore class="my-5">
                            ...
                            <textarea wire:model="beforeText" id="beforeText" name="beforeText" rows="8"
                                      class="block p-2.5 w-full text-xs bg-gray-50 rounded-[4px] border border-gray-300">{{$beforeText}}</textarea>
                        </div>
                        <div class="my-5">
                                                    ...
                                                    <td class="px-4 py-4 w-[200px] align-top">
                                                        <div class="relative">
                                                            <input wire:model="ingLists.{{$i}}.inglists.3.name" type="text"
                                                                   disabled
                                                                   class="@if(!empty($ingLists[$i]['inglists'][3]['bd']) && $ingLists[$i]['inglists'][3]['bd'] == 'net') bg-red-200 @else bg-gray-50 @endif block h-7 py-[0.5px] px-[10px] w-full text-[12px] text-gray-900 border border-gray-300  rounded-[2px] ">
                                                    ...
                                                    @if (!empty($stepLists[$i]['img']))
                                                        <div class="flex flex-col justify-center items-center">
                                                            @foreach($stepLists[$i]['img'] as $imgKey => $img)
                                                                <div class="relative">
                                                                    <img class="w-[140px] h-[140px] object-cover bg-gray-50 mb-2 border-2 border-black rounded-[4px]"  src="@if (is_object($img)) {{ $img->temporaryUrl() }} @else {{ asset('storage/'.$img) }}  @endif">
                        ...
                        <div wire:ignore class="my-5">
                            ...
                            <textarea wire:model.lazy="afterText" id="afterText" name="afterText" rows="8"
                                      class="block p-2.5 w-full text-xs bg-gray-50 rounded-[4px] border border-gray-300 focus:outline-none">{{$afterText}}</textarea>
                        </div>
                        <div class="my-5">
                            ...
                            @if ($recipeImg)
                                <img class="w-[200px] h-[200px] object-cover relative block bg-gray-50 border border-gray-300 rounded-[4px]"
                                     src="@if (is_object($recipeImg)) {{ $recipeImg->temporaryUrl() }} @else {{ asset('storage/'.$recipeImg) }}  @endif">

                            @else
                                <div
                                    class="w-1/5 h-[200px] relative block bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-[4px]"
                    ...
                    <div class="col-span-1">
                        <div class="sticky sticky-element top-5 w-full z-[49] border p-3 backdrop-blur-2xl ">
                            <div>
                                <select wire:model="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-[16px] rounded-[4px] focus:outline-none block w-full p-2">
                                    <option selected>Статус</option>
                                    <option value="2">Черновик</option>
                                    <option value="1">Опубликовать</option>
                                    <option value="6">На утверждении</option>
                                    <option value="4">Корзина</option>
                                </select>
                            </div>
                            <div class="flex justify-start items-center mt-4">
                                <i class="ti ti-calendar-due mr-2"></i> <span class="text-[16px]">{{ date("d.m.Y", strtotime($createDate)) }}</span>
                            </div>
                            <div class="mt-3 ml-0.5">
                                <div class="flex items-center mb-1">
                                    <input wire:model.live="checked" id="checked" name="checked" value="{{$checked}}" type="checkbox" @if($checked) checked @endif class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded cursor-pointer" >
                                    <label for="checked" class="ms-2 text-[16px] font-medium text-gray-900 dark:text-gray-300 cursor-pointer">Проверил</label>
                                </div>
                                <div class="flex items-center">
                                    <input wire:model.live="approv" id="approv" name="approv" value="{{$approv}}" type="checkbox" @if($approv) checked @endif class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded cursor-pointer">
                                    <label for="approv" class="ms-2 text-[16px] font-medium text-gray-900 dark:text-gray-300 cursor-pointer">Одобрил</label>
                                </div>
                            </div>
                            <hr class="my-3">
                            <div class="flex flex-col justify-between">
                                <div class="pb-3">Рейтинг:</div>
                                <div class="flex justify-between">
                                    <div>
                                        <input wire:model="positiveRating" type="number" min="0" class="w-[80px] h-7 text-gray-700 bg-green-200 font-light">
                                    </div>
                                    <div>  <span class="mx-2">/</span>  </div>
                                    <div>
                                        <input wire:model="negativeRating" type="number" min="0" class="w-[80px] h-7 text-red-700 bg-red-200 font-light">
                                    </div>
                                </div>
                            </div>
                            <hr class="my-3">
                            <div class="flex flex-col justify-between">
                                <div class="pb-3">Автор:</div>
                                <div class="flex justify-between">
                                    <select wire:model="author" class="bg-gray-50 border border-gray-300 text-gray-900 text-[14px] rounded-[4px] focus:outline-none block w-full py-1 px-2">
                                        @if($authors)
                                            @foreach($authors as $author)
                                                <option value="{{$author->id}}">{{$author->name}}</option>
                                            @endforeach
                                        @else
                                            <option>Авторв нет</option>
                                        @endif
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-between gap-2">
                                <button wire:loading.attr="disabled" wire:target="basket({{ $recipeIdForDel }})" wire:click="basket({{ $recipeIdForDel }})" type="button" class="w-1/2 px-3 py-2 text-xs font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:outline-none"><svg wire:loading wire:target="basket({{ $recipeIdForDel }})" fill='none' class="w-4 h-4 animate-spin mr-2" viewBox="0 0 32 32" xmlns='http://www.w3.org/2000/svg'>
                                        <path clip-rule='evenodd'
                                              d='M15.165 8.53a.5.5 0 01-.404.58A7 7 0 1023 16a.5.5 0 011 0 8 8 0 11-9.416-7.874.5.5 0 01.58.404z'
                                              fill='currentColor' fill-rule='evenodd' />
                                    </svg><span wire:loading.remove wire:target="basket({{ $recipeIdForDel }})">удалить</span></button>
                                <button wire:loading.attr="disabled" wire:target="updateRecipe" type="submit" class="w-1/2 px-3 py-2 text-xs font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:outline-none"><svg wire:loading wire:target="updateRecipe" fill='none' class="w-4 h-4 animate-spin mr-2" viewBox="0 0 32 32" xmlns='http://www.w3.org/2000/svg'>
                                        <path clip-rule='evenodd'
                                              d='M15.165 8.53a.5.5 0 01-.404.58A7 7 0 1023 16a.5.5 0 011 0 8 8 0 11-9.416-7.874.5.5 0 01.58.404z'
                                              fill='currentColor' fill-rule='evenodd' /><span wire:loading.remove wire:target="updateRecipe">сохранить</span></button>
                            </div>
                        </div>
